...
function checkAnswer(input){
    ...
    if (input ==a[1]){
        cori.innerHTML = "correct";
        cori.innerHTML = cori.innerHTML+ "<div><button id=\"next\">next</button></div>"
        next = document.getElementById("next");
        next.addEventListener("click",goCorrect);
    }
    else{
        
        cori.innerHTML = "incorrect";
        cori.innerHTML = cori.innerHTML+ "<div><button id=\"next\">next</button></div>"
        cori.innerHTML = cori.innerHTML+ "<div><button id=\"override\">override</button></div>"
        next = document.getElementById("next");
        next.addEventListener("click",goIncorrect);
        override = document.getElementById("override");
        override.addEventListener("click",goCorrect);
